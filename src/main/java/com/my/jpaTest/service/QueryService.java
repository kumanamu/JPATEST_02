...
//멤버 테이블중에서 씨름팀에 속해 있는 멤버의 이름만 출력
    public List<Member> fineMemberSsrium(){
        String sql = "SELECT m.name FROM Member m" + "WHERE m.team.teamName LIKE :teamName";
...
//씨름팀 인원수 구하기
    public Long teamCount(){
        String sql = "SELECT COUNT(m) FROM Member m" + "WHERE m.team.teamName LIKE :teamName";
    ...
    public  List<MemberDto> getMemberDto(){
        String sql = "SELECT NEW" + "com.my.jpaTest.dto.MemberDto(m.name, m.team.teamName)" + "FROM Member m";
